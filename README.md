...
- **ContextPack** — Automatic system prompt assembly (Lite/Standard/Full) with plan, findings, rawq code search
...
- **Durable Job Registry** — Tracks running/completed/failed jobs; recovers from interrupted runs on restart
...
│  ├─ Sidebar — Projects / Chats / Roundtables / Files │
│  ├─ ChatPanel — Stream view + RoundtableView          │
│  └─ ContextPanel — Plans / Artifacts / Trace / Memos  │
├─────────────────────────────────────────────────────┤
│  Tauri 2 Host (Rust)                                 │
...
│   ├── src/db/              # SQLite schema + migrations (v1-v12)
│   └── Cargo.toml
├── src/                     # React frontend
│   ├── components/tunaflow/ # UI components
│   ├── stores/slices/       # Zustand store (6 slices)
│   ├── lib/                 # Utilities + constants
│   └── tests/               # Vitest tests
├── docs/
│   ├── plans/               # Implementation plans (27 docs)
