...
## 제안 C: RT resume_token 확장 (Claude)

### 현재 상태

- 메인 채팅 Claude: ✅ `conversations.resume_token` 저장 → `--resume` 전달 → 서버측 세션 유지
- RT 전 엔진: ❌ `RunInput.resume_token = None` 하드코딩 (`executor.rs`)
- 설계 전제: "각 participant가 독립 세션" → resume 불필요

### 적용 가능 시나리오

Sequential RT에서 **같은 Claude participant가 라운드 간 세션을 이어가는** 경우:

```
라운드 1: Alice(claude) 실행 → session_id 반환 → 메모리에 저장
라운드 2: Alice(claude) 실행 → --resume session_id → 서버가 R1 자기 응답 기억
         → transcript 중 Alice 부분 재전송 불필요
```

### 득실 분석

```
3명 × 2라운드 Sequential RT:

현재:  6요청 × ~15k 토큰 = ~90k 토큰
resume 적용 시:
  R1: 3요청 × 15k = 45k (첫 라운드, resume 없음)
  R2: 3요청 × ~10k = 30k (본인 이전 응답 ~5k 절감)
  총: ~75k 토큰
...
- **Claude만 가능** — Codex/Gemini/OpenCode/Ollama는 resume 미지원
- **다른 participant의 새 응답은 여전히 prompt에 포함** 필요 — 절감되는 건 "본인의 이전 응답" 부분만
- **ContextPack 고정비용(identity/skills/rawq)은 매번 전송** — resume로 절감 불가
- **세션 만료 시 fallback 필요** — graceful degradation (새 세션 + 전체 context)
- **Deliberative(병렬) 모드에서는 효과 제한적** — 같은 participant의 다음 라운드에서만 의미

### tunapi 참고

tunapi는 resume_token을 **모든 경로에서 활용**:
- per-channel 세션 영속 (`chat_sessions.json`)
- Session Lock (동일 세션 동시 실행 방지, `WeakValueDictionary` 세마포어)
- 채팅 메시지에 resume 토큰 embed → 사용자 reply 시 자동 추출

tunaFlow의 메인 채팅은 이미 비슷한 구조(DB 저장 + 자동 전달)이지만, RT에서는 미적용.

### 구현 범위

- `executor.rs`: `stream_participant()`에 resume_token 파라미터 추가
- `execute_sequential()`: 라운드 간 participant별 session_id 맵 유지 (`HashMap<String, String>`)
- `ParticipantResult`에 `session_id: Option<String>` 필드 추가
- `RunOutput.session_id` → 맵에 저장 → 다음 라운드에서 전달
- 세션 만료 시 fallback: resume 실패하면 None으로 재시도

### 우선순위 판단

Tiering(제안 A)과 벡터 맥락 공유(제안 B)가 전 엔진에 적용 가능하고 절감 효과가 더 크므로(~70% vs ~17%), resume 확장은 **Tiering 이후 추가 최적화**로 적합.

---

## 제안 D: sqlite-vec 도입 (기존 제안 C)
