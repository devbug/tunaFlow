...
- **axum은 Tauri 앱 내부에서 `tokio::spawn`으로 기동**. 별도 바이너리가 아님.
- `DbState` (Arc<Mutex<Connection>>)를 axum 라우터의 State로 공유 — 추가 DB 연결 불필요.
- Tauri 이벤트 → WS 브릿지: `AppHandle.listen("agent:completed")` → `tokio::sync::broadcast` → WS 클라이언트.
- 기존 `mcp-server/index.mjs` (Node.js)는 Phase 3에서 Rust 네이티브 MCP로 교체. 그 전까지는 병행 운영.
...
| **에이전트 비용**: E2E에서 실제 에이전트 호출 | `dryRun: true` 모드 (프롬프트 조립까지만, CLI 미실행) + ollama localhost 모델 (비용 0) 경로 |
| **WS 이벤트 브릿지**: Tauri 이벤트→WS 포워딩 | `AppHandle.listen` → WS broadcast (~50줄) |
| **쓰기 동시 접근**: API + UI 동시 쓰기 | DbState.write Mutex로 직렬화 (WAL 모드 읽기는 동시 가능). 부하 테스트 시 확인 |
