...
- **Auto Fix**: Quick Wins의 "Fix" 버튼 → 자동 수정 → 테스트 → 검증 → 롤백 or 완료
- **Send to Architect**: guided/strategic 항목 요약 → Architect에게 Plan 생성 요청
...
#### Auto 수정 파이프라인 (CodeCureAgent 패턴 적용 [4])

```
1. finding 기반 수정 실행 (에이전트)
2. run_project_tests → 기존 테스��� 통과 확인
3. rawq 재스캔 → 원래 패턴 사라졌는지 확인
4. 새 경고/에러 없는지 확인
5. 실패 시 → 자동 롤백 + guided로 등급 상향
```
...
| **E** | Insight 탭 UI (Quadrant 뷰, 체크박스, Auto Fix, Send to Architect) | medium |
| **F** | Auto 수정 파이프라인 (실행 → 테스트 → 검증 → 롤백) | medium |
