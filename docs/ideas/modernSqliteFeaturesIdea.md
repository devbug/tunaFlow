...
PRAGMA mmap_size = 268435456;     -- 메모리 맵 256MB. 큰 DB에서 읽기 가속
```
...
| `mmap_size` | 큰 DB에서 읽기 가속 | 플랫폼 호환성 | 나중 |
...
**비용이 높습니다.** messages 테이블은 FTS5 트리거, 24개 인덱스 중 다수가 연결되어 있어 마이그레이션이 복잡합니다.
...
| 항목 | 이유 |
|------|------|
| 기존 테이블 STRICT 전환 | 마이그레이션 비용 과도 (FTS5 트리거 + 24개 인덱스 재생성) |
