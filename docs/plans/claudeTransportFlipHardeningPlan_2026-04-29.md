...
3. **rate_limit_event 미가시화**: backend stream-json 에 `rate_limit_event` payload 가 들어오지만 frontend 에 노출 안 됨. 사용자가 본인 한도 / overage 상태 / reset 시점 모름

본 plan 은 셋을 묶어 v0.1.5-beta 의 release blocker 로 처리.
...
**Phase 1 (P0, v0.1.5-beta release blocker)**: Task 01~04 — auto fallback + ContextPack revival + UI 가시화. 외부 사용자 onboarding 좌절 차단의 핵심.
