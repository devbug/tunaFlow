...
- [claudeSdkSessionWindowGuardDeveloperHandoff_2026-05-09](./claudeSdkSessionWindowGuardDeveloperHandoff_2026-05-09.md) — **P0 ready 핸드오프 (사용자 본인 환경 회귀)**. Plan: `claudeSdkSessionWindowGuardPlan_2026-05-09`. Reviewer "Prompt is too long" 회귀 — `MAX_TOTAL_PROMPT` outgoing-only 가드, claude SDK 세션 누적 history 미가드. accumulated_input_tokens 임계 (180K default / 900K `[1m]`) → 자동 fresh-rotate + toast + Reviewer specific squeeze. 6 task → 4 PR 분리. v0.1.8-beta minor. 외부 issue 없음 (in-session 보고).
