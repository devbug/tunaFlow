...
이 프로젝트는 단순히 사용자가 편한 채팅 앱을 만드는 것이 아니다.
더 중요한 기준은:

- 에이전트가 더 적은 마찰로 작업할 수 있는가
- 컨텍스트 품질이 높고 일관적인가
- 불필요한 토큰 낭비 없이 필요한 기억만 전달되는가
- 사람용 UI보다 agent workflow 효율이 개선되는가

즉 기능 우선순위는 기본적으로 `agent-first` 관점에서 판단한다.
...
### Claude 결과 검토 후 기본 규칙

1. 사용자가 Claude 결과를 붙여주면 먼저 의견을 짧게 말한다
2. 큰 이견이나 우선순위 충돌이 없으면, 사용자의 추가 확인을 기다리지 말고 바로 다음 단계 문서/프롬프트를 준비한다
3. 사용자가 별도 코멘트로 방향을 바꾸지 않는 한, `자동으로 다음 우선순위 진행`이 기본 동작이다

즉 현재 워크플로는:

- Claude가 구현
- Codex가 검토
- 이견 없으면 Codex가 다음 단계 계획/프롬프트를 자동 준비
