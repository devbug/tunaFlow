//! Memory compression helpers — LLM summarization, transcript pruning, topic writing.
//!
//! Contains the full compression pipeline:
//!   1. `needs_compression` — threshold check
//!   2. `build_transcript` — extract + prune older messages
//!   3. `compress_memory_blocking` — call Claude, persist topics
//!
//! `conversation_memory.rs` imports from this module (one-way dependency).
...
pub(crate) fn parse_topics(raw: &str) -> Vec<MemoryTopic> {
...
pub(crate) fn build_transcript(conn: &Connection, conversation_id: &str) -> Result<(String, i64), AppError> {
    let total: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM messages WHERE conversation_id = ?1",
            [conversation_id],
...
mod tests {
    ...
    fn prune_six_line_code_block_truncated() {
        let input = "```\n1\n2\n3\n4\n5\n6\n```\nafter";
        let result = prune_for_summary(input);
        assert!(result.contains("[... 3 lines pruned]"));
    }
