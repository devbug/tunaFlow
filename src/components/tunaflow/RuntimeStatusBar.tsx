...
export function RuntimeStatusBar() {
  ...
  // Aggregate cost from conversation
  useEffect(() => {
    if (!selectedConversationId) { setTotalCost(0); return; }
    invoke<{ totalCostUsd: number }[]>("list_trace_spans", { conversationId: selectedConversationId })
      .then((spans) => {
        const cost = (spans as any[]).reduce((sum, s) => sum + (s.costUsd ?? 0), 0);
        setTotalCost(cost);
      })
      .catch(() => setTotalCost(0));
  }, [selectedConversationId, runningThreadIds.length]);
