import { Users, Trash2, Plus, GitBranch } from "lucide-react";
...
interface RoundtablesSectionProps {
  ...
  openBranchStream: (branchId: string) => void;
...
export function RoundtablesSection({
  ...
  selectConversation, renameConversation, openThread, openBranchStream,
  handleDelete, handleRenameBranch, onDeleteBranch, onCreateRT,
    ...
    return (
      <div key={b.id}>
        <TreeRow depth={depth} active={isActive} isParent={children.length > 0}
          icon={<GitBranch className="w-3.5 h-3.5 text-agent-gemini/40" />}
          label={
            <span className="flex items-center gap-1">
              <InlineRename value={b.customLabel ?? b.label} onSave={(v) => handleRenameBranch(b.id, v)} inputClassName="text-[10px] w-full" />
              <span className="text-[7px] text-agent-gemini/40 shrink-0">branch</span>
            </span>
          }
    ...
    <>
      ...
      {rtOpen && (
        <>
          ...
          {rtConvs.map((conv) => {
            ...
            return (
              <TreeRow key={conv.id} depth={1} active={isActive}
                ...
                label={
                  <span className="flex items-center gap-1">
                    <InlineRename value={conv.customLabel ?? conv.label} onSave={(v) => renameConversation(conv.id, v)} inputClassName="text-[10px] w-full" />
                  </span>
                }
          ...
          {/* RT branches — visually distinct from RT conversations */}
