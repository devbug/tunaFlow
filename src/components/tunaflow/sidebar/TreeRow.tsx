...
export function TreeRow({
  ...
  return (
    <div onClick={onClick}
      ...
      <span className="flex-1 min-w-0 text-[11px] font-medium truncate">{label}</span>
      {suffix}
      {actions && (
...
export function SectionHeader({ title, expanded, onToggle, actions, className }: {
  ...
  return (
    <div onClick={onToggle}
      ...
      <span className="text-[11px] font-semibold text-sidebar-foreground/60 pl-1.5 flex-1">{title}</span>
